...
The harness builds its corpora from third-party sources download on demand into
the gitignored repo-root `.cache/bench/`, hash-verified against the pinned manifests
in [`sources/`](sources/). Only those manifests (url + sha256 + license) are comitted.

| Asset | Use | License | Pinned in |
|-------|-----|---------|-----------|
...
- **dwyl/english-words** — a public-domain word list. The harness lowercases, keeps
  ASCII words of length 3–15, and samples them with a Zipfian distribution.
