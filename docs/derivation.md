...
The familiar summed-IDF overlap $\sum_{g\in Q\cap d}\mathrm{idf}_g$ is its rare-gram limit. Dropping $\log(1-p_g)$ leaves the surprisal $-\ln p_g = \mathrm{idf}_g$ [Shannon1948; SparckJones1972], equal to $E_g$ in the rare tail and diverging only for common grams. The gap is $-\log(1-p_g)$ — about $0.36$ nats at $p_g = 0.3$ and a full $0.69$ at $p_g = 0.5$, where a gram in half the corpus carries zero discriminative evidence that surprisal would still score at $0.69$. The matched sum is the matched part of the query's cross-entropy against the corpus marginal [LaffertyZhai2001], and logit-idf is the consistent likelihood-ratio choice with surprisal as its approximation.
...
Finally, the count-credit and length terms are post-accumulation floats, so any top-$k$ truncation must be applied after them, on the corrected scores — never on the bare bit-sliced sum, which would truncate on the wrong ordering. The post-pass is cheap enough to run over the full candidate set first: the work budget $\sum_{g\in P} df_g \le C$ upper-bounds the candidate union ($\#\text{candidates} \le \sum df_g \le C$), so the post-pass is $O(C)$ and the inner loop $O(C\cdot\text{planes})$. "Near-constant work per query" means "work bounded by the df budget."
...
[Cantelli1928] F.P. Cantelli. The one-sided Chebyshev (Cantelli) inequality, 1928.
