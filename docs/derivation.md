...
score_query(query, channel):
    ...
    for rv in rank_views:                                            # rv = a RANK (PRIMARY / SECONDARY), pooling all scripts at that rank
        ...
        #     whose only grams are common (df > C) is abundantly represented already, left to phase 2 -- this
        #     keeps the floor's total added work bounded (~ one cheap walk per script).
