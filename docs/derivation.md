...
| $E_g = \ln\dfrac{N - df^{\mathrm{eff}}_g + \kappa}{df^{\mathrm{eff}}_g + \kappa}$ | per-gram energy: the RSJ log-odds (logit-idf) of the smoothed, floored estimate — the **exact** Beta($\kappa,\kappa$) posterior-mean form, finite for every $df \le N$ (v0.5; §4) — used clamped as $\max(0,E_g)$. Its rare-gram limit is the surprisal $\mathrm{idf}_g = \ln\frac{N}{df^{\mathrm{eff}}_g+\kappa}$ |
...
**Estimation.** The maximum-likelihood estimate $\hat p_g = df_g/N$ is overconfident in the rare tail: a gram seen once has a wide posterior on its true rarity. Jeffreys smoothing [Jeffreys1946] corrects it to the exact Beta($\kappa,\kappa$) posterior mean $\hat p_g = (df_g + \kappa)/(N + 2\kappa)$, and the energy is the log-odds of the corrected estimate:

$$E_g = \ln\frac{1-\hat p_g}{\hat p_g} = \ln\frac{N - df_g + \kappa}{df_g + \kappa}, \qquad \kappa = 0.5,$$

with rare-gram limit the smoothed surprisal $\mathrm{idf}_g = \ln\frac{N}{df_g+\kappa}$. This $E_g$ is the standard Robertson–Spärck-Jones relevance-free term weight [RSJ1976], not a fresh construction, with the smoothing in the well-studied family of language-model smoothing for IR [ZhaiLafferty2004]. The smoothing is always applied. It noticeably affects the continuous quantities (the pruning budget of §5) and is nearly invisible to coarsely-quantized weights. (Pre-v0.5 the numerator carried $-\kappa$ — the log-odds of the *unnormalized* $(df_g+\kappa)/N$ — which is undefined for $df_g \ge N-\kappa$ and forced a $-\infty$ special case for ubiquitous grams. The exact posterior form is finite for every $df_g \le N$: at $df_g = N$ it is $\ln\frac{\kappa}{N+\kappa}$, deeply negative and clamped to weight zero like any common gram. The two differ appreciably only in that common tail, which the $\max(0,\cdot)$ clamp zeroes, and by $O(\kappa/N)$ in the rare tail.)
...
$$\mu \le \max\!\left(0,\ \frac{\max(0,E_{\text{top}}) - \sum_{g\in\text{common}} \max(0,E_g)}{\#\text{common} - 1 + \mathbb{1}[E_{\text{top}}\text{ floored}]}\right),$$

restricted to the query-relative commons, so that a query spread across several comparable rare grams — which has no dominant member, hence no commons under the relative definition — remains uncapped.

**The denominator tracks the anchor's own credit (v0.5).** The cap is the largest $\mu$ under which the on-topic document (the one carrying the dominant gram) still out-scores a commons-only document. A **non-floored** dominant gram itself earns $\mu$, so the governing inequality is $E_{\text{top}} + \mu \ge \sum_c E_c + \#c\cdot\mu$, giving the $\#c - 1$ denominator. A **floored** anchor — the common case, since $df_{\min}=\sqrt N$ floors genuinely-rare real grams — earns **no** credit (the withholding above), the inequality is $E_{\text{top}} \ge \sum_c E_c + \#c\cdot\mu$, and the tight cap divides by $\#c$. Using $\#c - 1$ unconditionally (the pre-v0.5 form) over-credits a commons-only document by exactly one $\mu$ past the floored-anchor document — a recorded ranking inversion (an off-topic commons document at 8.09 over the on-topic anchored one at 6.20) whose mechanism is precisely this missing $\mu$. The indicator restores the protection while keeping floored anchors *in* $E_{\text{top}}$, per the paragraph below. No tie ambiguity arises at the argmax: a floored gram carries the realized floored energy, strictly above every non-floored gram's, so the top cannot straddle the floored/non-floored boundary.
...
energy(df_g, channel, N):                                              # RSJ log-odds (logit-idf); surprisal is the rare-gram limit
    ...
    return ln( (N - df_eff + KAPPA) / (df_eff + KAPPA) )               # exact Jeffreys posterior log-odds (Section 4); KAPPA = 0.5;
    #                                                                   #   < 0 for p>0.5 (zeroed by max(0,.) at use), FINITE even at df = N

score_query(query, channel):
    # Tokenize into grams tagged with script + order; a mixed-script query yields several orders. Word
    # boundaries come from WHITESPACE (the operative choice: under-splitting only raises the stop's
    # variance, so it is the recall-safe subset; delimiter-punctuation breaking is a deferred precision
    # refinement, Section 8/11), and the same breaks apply to gram windows so no gram straddles two
    # words (Section 8/4). RANK-VIEWS, not absolute orders: grams_at_rank(PRIMARY) pools every
    ...
    for rv in rank_views:                                            # rv = a RANK (PRIMARY / SECONDARY), pooling all scripts at that rank
        ...
        C    = (N <= k or dbar not in (1, N-1)) ? +inf              # recall-safe guards -> unbounded, never too-tight
             : (1/r) * ln(N/k) * dbar / ln(N/dbar)                  # r = the channel reliability (query-side: SIGMA)
        ...
        def admit(g):                                              # add g to P and fold it into the stop's running mean/variance
            ...
            if (channel == DOC_SIDE or not g.floored) and max(0, g.E) > 0:   # floored AND clamped-zero-energy grams carry no
                #                                                    #   identification power: neither may form or grow a stop block —
                #                                                    #   a zero-energy block would inflate the block count B and loosen
                #                                                    #   the recall-safety gate r >= c^2/(B+c^2) (Section 5)
        ...
        # concentration_cap(P) = (E_top - sum_commons) / (#commons - 1 + (E_top floored ? 1 : 0)):
        # a floored anchor earns no mu itself, so the tight Section 9 inequality divides by #commons.
...
### Deviations from §12 in the realization (tracked)

The implementation is faithful to this algorithm with the following **known, deliberate deviations**,
each either a recall-safe narrowing or a deferred milestone. They are listed here so the doc-vs-code
gap is explicit rather than silent.

| # | §12 says | The realization does | Status |
|---|---|---|---|
| 1 | `loosen_budget_or_restore_reserve()` on `collected_energy_far_below` (§5's corroboration reserve: restore pruned mid-rarity reals before falling to the shorter order) | Budget-starvation routes straight to the secondary rank-view (the same signal gates `starved(s)`) — the reserve is never restored | Open; schedule against benchmark evidence |
| 2 | Smooth shrink of $\mu$ toward the concentration cap (§9 prefers it in tuning) | Hard `min(mu, cap)` | Open tuning refinement |
| 3 | Doc-side channel: per-field $\varepsilon$, no floor, per-gram $r = \sigma(1-\varepsilon)^n$ | Query-side channel only ($r = \sigma$ uniform) | Lands with the field-aware index milestone (per-field posting partitions) |
| 4 | Word breaks on whitespace + delimiter punctuation (the original §8/§12 comment) | **Whitespace only** — the recall-safe subset (under-splitting only raises the stop's variance); §12 above now states the operative choice | Ratified; delimiter breaking is a deferred precision refinement |
| 5 | `starved(s)` falls `PRIMARY → SECONDARY` whenever every rank-view lacks an in-corpus gram (the literal while-loop) | A query that *produced* primary grams which are all absent (df = 0) stays a no-match — it does not fuzzily fall to the far-less-selective shorter order (the no-match precision contract) | Ratified narrowing |
| 6 | `starved(s)` is per script-class | For the `Common` class the structural trigger is **word-granular**: an interior digit/symbol fragment of a strong-script run (whose word has primary coverage) is not starvation; a standalone digit word is | Ratified (v0.5) — the per-class literal spuriously fused clean digit-bearing queries |
| 7 | $\Delta H$ from df-distribution entropies $H$ | $H = \ln V$ (log vocabulary size), the max-entropy proxy — see §8 | Ratified realization; full $H$ is a drop-in refinement |
| 8 | Selection keeps the typo floor $F = m + d$ (`d` a per-typo damage constant) *nowhere in this derivation* — §5's selection is per-class floor + rarest-first until stop/budget | The realization retains a legacy typo floor $F = m + d$, $d = 4$, budget-aware — the last pre-derivation tuned constant | Open; dissolve or derive once the benchmark harness gates it (§4's $(m-\nu)/n$ note points the way) |
