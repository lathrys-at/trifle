...
$$\text{stop when } \sum_{g\in P} r\,\max(0,E_g) \;-\; c\,\sigma_{\text{match}} \;\ge\; \ln(N/k), \qquad \sigma_{\text{match}}^2 = \sum_g r(1-r)\max(0,E_g)^2 + \sum_{g\ne h} \max(0,E_g)\max(0,E_h)\, \operatorname{Cov}(B_g, B_h).$$
...
$$\sigma_{\text{match}}^2 = r(1-r)\Big(\sum_{g\in P} \max(0,E_g)^2 \;+\; 2\!\!\sum_{\substack{(g,h)\\ d=|\Delta\mathrm{pos}|<n}}\!\! \varphi_d\,\max(0,E_g)\max(0,E_h)\Big), \qquad \operatorname{Cov}(B_g, B_h) = \varphi_d\,r(1-r).$$
