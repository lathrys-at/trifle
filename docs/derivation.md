...
score_query(query, channel):
    ...
    for rv in rank_views:                                            # rv = a RANK (PRIMARY / SECONDARY), pooling all scripts at that rank
        ...
        #     whose only grams are common (df > C) is non-discriminating and budget-excluded like any df>C gram
        #     -- this keeps the floor's total added work bounded (~ #classes*C; cheap when classes have a rare gram).
        floor_seats = [ first g per distinct (g.script, g.n) with g.df <= C, in z-score order ]   # >= 1 present gram per representable class
        for g in floor_seats: admit(g)                             # UNCONDITIONAL (even past C and the stop) -- generalizes the typo floor; per-class representation invariant
