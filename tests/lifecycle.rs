//! Write lifecycle: upsert-replace, delete, monotonic ids, compaction, rebuild, stats, and
//! persistence across reopen.

mod common;
use common::*;
use trifle::tokenize::DefaultTokenizer;
use trifle::{Config, Document, Index, Schema};

fn finds(h: &Harness, q: &str, doc: i64) -> bool {
    hit(&h.search(q, 10).unwrap(), doc)
}

/// A one-segment `Document` under `label`.
...
fn upsert_replaces_a_segment() {
    let h = Harness::new();
    h.put(1, "f", "obsolete antiquated terminology");
    assert!(finds(&h, "obsolete antiquated", 1));
    h.put(1, "f", "modern contemporary vocabulary");
...
fn upsert_one_label_leaves_other_labels_intact() {
    let h = Harness::new();
    h.put(1, "f", "alpha bravo charlie");
    h.put(1, "scan", "delta echo foxtrot");
    h.put(1, "f", "golf hotel india");
...
fn removed_content_is_unfindable_before_any_compaction() {
    let h = Harness::new();
    h.put(1, "f", "zugzwang quixotic jamboree");
    h.remove(1);
    assert!(h.search("zugzwang quixotic", 10).unwrap().is_empty());
}

#[test]
fn freed_ids_are_not_reused_so_no_false_positives() {
    let h = Harness::new();
    h.put(1, "f", "wxyzqj uniquetoken alpha");
    h.remove(1);
    h.put(2, "f", "entirely separate beta gamma delta");
    assert!(
...
fn search_works_across_base_and_delta_after_compact() {
    let h = Harness::new();
    h.put(1, "f", "established baseline content");
...
fn rebuild_reclaims_a_grown_id_space() {
    let h = Harness::new();
    for i in 0..20 {
        h.put(1, "f", &format!("revision number {i} of the document"));
    }
    h.index
...
fn data_survives_reopen_of_the_same_file() {
    ...
    let path = h.db_path();
    drop(h.index);
    let reopened: Index<DefaultTokenizer> =
        Index::open_at(&path, Schema::flat(), Config::default()).unwrap();
    let hits = reopened
        .reader()
        .unwrap()
        .matches("quick brown fox", &trifle::SearchOpts::new(), 10)
        .unwrap();
...
#[test]
fn upsert_keeps_every_segment_of_a_doc() {
    let h = Harness::new();
    h.upsert(
        1,
        &[
...
fn writer_batch_accumulates_interleaved_docs() {
    let h = Harness::new();
    // Doc 1's two labels straddle a different doc in the same writer batch.
    let mut w = h.index.writer().unwrap();
    w.upsert(1, &[("a", "alpha distinctive content")]).unwrap();
    w.upsert(2, &[("z", "separate document entirely")]).unwrap();
    w.upsert(1, &[("b", "omega distinctive content")]).unwrap();
    w.commit().unwrap();
...
fn upsert_replaces_only_the_named_docs() {
    let h = Harness::new();
    h.put(1, "f", "obsolete antiquated");
    h.put(2, "f", "deprecated archaic");
    h.put(3, "f", "untouched keepsake permanent");
    let mut w = h.index.writer().unwrap();
...
fn remove_segment_wipes_one_label_and_leaves_the_others() {
    let h = Harness::new();
    h.put(1, "scan", "alpha ocr distinctive");
    h.put(1, "alt", "beta caption distinctive");
    h.put(2, "scan", "gamma other document");
...
#[test]
fn rebuild_skips_empty_documents() {
    // A segment-less document materializes no rows (no doc table for a ghost to live in).
    let h = Harness::new();
    h.index
        .rebuild(vec![
...
fn rebuild_rejects_duplicate_keys() {
    let h = Harness::new();
    h.put(1, "f", "preexisting live content");
    let err = h
        .index
        .rebuild(vec![
...
fn rebuild_on_an_empty_corpus_empties_and_stays_usable() {
    ...
    assert_eq!(h.index.stats().unwrap().segments, 0);
    assert!(h.search("quick brown", 10).unwrap().is_empty());
    h.put(1, "f", "freshly inserted after empty rebuild");
...
fn stats_segments_track_upsert_and_remove() {
    let h = Harness::new();
    h.put(1, "f", "one segment here");
    assert_eq!(h.index.stats().unwrap().segments, 1);
    h.put(1, "f", "replaced single segment"); // upsert: still 1
    assert_eq!(h.index.stats().unwrap().segments, 1);
    h.put(1, "s", "a second label segment"); // +1
    assert_eq!(h.index.stats().unwrap().segments, 2);
...
fn results_are_byte_identical_across_reopen_and_rebuild() {
    ...
    let baseline = h.search(q, 10).unwrap();
